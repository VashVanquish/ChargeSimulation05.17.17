...
class Charge():
    ...
    def accel2(self, charge):
        if self.charge < 0:
            i = -1
        else:
            i = 1
        if self.getRadius(charge.getPos()[0],charge.getPos()[1]) > 1225:
            if (self.x + 12) != (charge.getPos()[0] + 12) and (self.y + 12) != (charge.getPos()[1] + 12):
                if (self.x + 12) > (charge.getPos()[0] + 12) and (self.y + 12) < (charge.getPos()[1] + 12):
                    angle = math.atan((((self.y + 12))-(charge.getPos()[1] + 12))/((charge.getPos()[0] + 12)-(self.x + 12)))
                    charge.changeXSpeed(math.cos(angle) * i * -60/(self.getRadius(charge.getPos()[0],charge.getPos()[1]) * charge.getMass()))
                    charge.changeYSpeed(math.sin(angle) * i * 60/(self.getRadius(charge.getPos()[0],charge.getPos()[1]) * charge.getMass()))
                elif (self.x + 12) < (charge.getPos()[0] + 12) and (self.y + 12) < (charge.getPos()[1] + 12):
                    angle = math.atan((((self.y + 12))-(charge.getPos()[1] + 12))/((self.x + 12)-(charge.getPos()[0] + 12)))
                    charge.changeXSpeed(math.cos(angle) * i * 60/(self.getRadius(charge.getPos()[0],charge.getPos()[1]) * charge.getMass()))
                    charge.changeYSpeed(math.sin(angle) * i * 60/(self.getRadius(charge.getPos()[0],charge.getPos()[1]) * charge.getMass()))
                elif (self.x + 12) < (charge.getPos()[0] + 12) and (self.y + 12) > (charge.getPos()[1] + 12):
                    angle = math.atan((((charge.getPos()[1] + 12)-(self.y + 12)))/((self.x + 12)-(charge.getPos()[0] + 12)))
                    charge.changeXSpeed(math.cos(angle) * i * 60/(self.getRadius(charge.getPos()[0],charge.getPos()[1]) * charge.getMass()))
                    charge.changeYSpeed(math.sin(angle) * i * -60/(self.getRadius(charge.getPos()[0],charge.getPos()[1]) * charge.getMass()))
                elif (self.x + 12) > (charge.getPos()[0] + 12) and (self.y + 12) > (charge.getPos()[1] + 12):
                    angle = math.atan((((charge.getPos()[1] + 12)-(self.y + 12)))/((charge.getPos()[0] + 12)-(self.x + 12)))
                    charge.changeXSpeed(math.cos(angle) * i * -60/(self.getRadius(charge.getPos()[0],charge.getPos()[1]) * charge.getMass()))
                    charge.changeYSpeed(math.sin(angle) * i * -60/(self.getRadius(charge.getPos()[0],charge.getPos()[1]) * charge.getMass()))
            else:
                if self.y == charge.getPos()[1]:
                    if self.x > charge.getPos()[0]:
                        charge.changeXSpeed( i * -60/(self.getRadius(charge.getPos()[0],charge.getPos()[1]) * charge.getMass()))
                    else:
                        charge.changeXSpeed( i * 60/(self.getRadius(charge.getPos()[0],charge.getPos()[1]) * charge.getMass()))
                else:
                    if self.y > charge.getPos()[1]:
                        charge.changeYSpeed( i * -60/(self.getRadius(charge.getPos()[0],charge.getPos()[1]) * charge.getMass()))
                    else:
                        charge.changeYSpeed( i * 60/(self.getRadius(charge.getPos()[0],charge.getPos()[1]) * charge.getMass()))
